...
@interface GroupChatComposeViewController () <HPGrowingTextViewDelegate,ParticipantsPopUpDelegate>{
    ...
    ParticipantsViewController *vwParticipants;
...
@implementation GroupChatComposeViewController
...
-(void)setUp{
    ...
    lblChatCount.text = _strChatCount;
    lblChatCount.clipsToBounds = YES;
    lblChatCount.layer.cornerRadius = 10.f;
    lblChatCount.layer.borderWidth = 1.f;
...
-(void)loadChatHistoryWithPageNo:(NSInteger)pageNo isByPagination:(BOOL)isPagination{
    ...
    [APIMapper loadChatHistoryWithGameID:_strGameID toUserID:nil page:pageNo success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
-(void)newChatHasReceivedWithDetails:(NSDictionary*)chatInfo{
    ...
    if (NULL_TO_NIL([chatInfo objectForKey:@"member_count"])) lblChatCount.text = [NSString stringWithFormat:@"%d",[[chatInfo objectForKey:@"member_count"] integerValue]]  ;
...
-(void)postMessage{
    ...
    if (textView.text.length > 0) {
        ...
        [APIMapper postChatMessageWithGameID:_strGameID toUserID:nil message:message success:^(AFHTTPRequestOperation *operation, id responseObject){
...
-(IBAction)showParticipants{
    
    if (!vwParticipants) {
        
        vwParticipants =  [UIStoryboard get_ViewControllerFromStoryboardWithStoryBoardName:StoryboardForSlider Identifier:StoryBoardIdentifierForParticipantsView];
        vwParticipants.delegate = self;
        vwParticipants.strGroupID = _strGameID;
        UIView *popup = vwParticipants.view;
...
-(void)showChatPageWithInfo:(NSDictionary*)details{
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        // animate it to the identity transform (100% scale)
        vwParticipants.view.transform = CGAffineTransformMakeScale(0.01, 0.01);
    } completion:^(BOOL finished){
        // if you want to do something once the animation finishes, put it here
        [vwParticipants.view removeFromSuperview];
        vwParticipants = nil;
        if (details) {
            ...
            if ([details objectForKey:@"name"]) {
                chatCompose.strUserName = [details objectForKey:@"name"];
            }
