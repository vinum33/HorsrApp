...
@interface PrivateChatComposeViewController () <HPGrowingTextViewDelegate>{
...
@implementation PrivateChatComposeViewController
...
-(void)loadChatHistoryWithPageNo:(NSInteger)pageNo isByPagination:(BOOL)isPagination{
    ...
    [APIMapper loadChatHistoryWithGameID:nil toUserID:_strUserID page:pageNo success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
-(void)newChatHasReceivedWithDetails:(NSDictionary*)chatInfo{
    ...
    lblTitle.text = [chatInfo objectForKey:@"name"];
...
-(void)postMessage{
    ...
    if (textView.text.length > 0) {
        ...
        [APIMapper postChatMessageWithGameID:nil toUserID:_strUserID message:message success:^(AFHTTPRequestOperation *operation, id responseObject){
