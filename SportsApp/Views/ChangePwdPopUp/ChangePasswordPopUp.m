...
-(void)submitClicked:(id)sender{
    ...
    [self checkAllFieldsAreValid:^{
        ...
        [APIMapper changePasswordWithCurrentPwd:strOldPwd newPWD:strNewPwd confirmPWD:strConfirmPwd success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            [self displayErrorMessgeWithDetails:task.responseData error:error];
...
-(void)displayErrorMessgeWithDetails:(NSData*)responseData error:(NSError*)error{
    ...
    if (strAPIErrorMsg)  [self showAlertWithTitle:@"Change Password" message:strAPIErrorMsg];
